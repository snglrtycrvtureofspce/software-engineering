...
namespace ApartmentExchange
{
    ...
    {
        private static HashSet<ExchangeApplication> cardIndex = new HashSet<ExchangeApplication>(); // Картотека (set)
        ...
        {
            using (InputForm inputForm = new InputForm("Ввод новой заявки"))
            {
                if (inputForm.ShowDialog() == DialogResult.OK)
                {
                    ExchangeApplication newApp = inputForm.GetApplication();
                    ...
                    {
                        MessageBox.Show("Такая заявка уже существует.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }
        ...
        {
            using (InputForm inputForm = new InputForm("Ввод заявки для поиска"))
            {
                if (inputForm.ShowDialog() == DialogResult.OK)
                {
                    ExchangeApplication newApp = inputForm.GetApplication();
                    ...
                    if (match != null)
                    {
                        MessageBox.Show($"Найден подходящий вариант:\n{match}\nЗаявка удалена из картотеки.", "Совпадение найдено", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    else
                    {
                        if (cardIndex.Add(newApp))
                        {
                            MessageBox.Show("Совпадение не найдено. Заявка добавлена в картотеку.", "Добавлено", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("Такая заявка уже существует.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
